...
import {
  ...
  Copy, Check, Activity, DollarSign, Store,
} from 'lucide-react'
...
export function AppWorkspace({
  ...
  const [view, setView] = useState<View>(() => queryView || initialView)
  const [activeTab, setActiveTab] = useState<Tab>(initialTab)
  const [browsing, setBrowsing] = useState<boolean>(() => queryBrowse || initialBrowsing || (queryView === null && initialBrowsing))
  ...
  function handleNavigate(v: View) {
    ...
    if (pathname === '/assistant') {
      // Navigating back to main app
      if (v === 'shop') {
        router.push('/app')
      } else {
        router.push(`/app?view=${v}`)
      }
      return
    }
